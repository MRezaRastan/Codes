...
#include "regionSizeDistributionR.H"
...
namespace Foam
{
    ...
    {
        defineTypeNameAndDebug(regionSizeDistributionR, 0);
        addToRunTimeSelectionTable
        (
            functionObject,
            regionSizeDistributionR,
            dictionary
        );
    }
}
...
void Foam::functionObjects::regionSizeDistributionR::writeAlphaFields
(
...
Foam::Map<Foam::label>
Foam::functionObjects::regionSizeDistributionR::findPatchRegions
(
    const regionSplit& regions
) const
{
...
Foam::functionObjects::regionSizeDistributionR::divide
(
...
void Foam::functionObjects::regionSizeDistributionR::writeGraphs
(
...
Foam::functionObjects::regionSizeDistributionR::regionSizeDistributionR
(
    const word& name,
    const Time& runTime,
...
bool Foam::functionObjects::regionSizeDistributionR::read(const dictionary& dict)
{
...
bool Foam::functionObjects::regionSizeDistributionR::execute()
{
    return true;
}


bool Foam::functionObjects::regionSizeDistributionR::write()
{
    ...
    if (allRegionVolume.size())
    {
        ...
        if (isoPlanes_)
        {
            ...
            scalarField sumSortedVols(nDownstreamBins_, Zero);	//////////////////////////////////////
            forAll(distToPlane, i)
            {
                if (downstreamIndices[i] != -1)
                {
                    binDownCount[downstreamIndices[i]] += 1.0;
                    sumSortedVols[downstreamIndices[i]] += sortedVols[i]; //////////////////////////////////////
                }
            }
            ...
            if (Pstream::master())
            {
                ...
                writer.write("number", binDownCount);
                writer.write("volume_vs_x", sumSortedVols); 	//////////////////////////////////////
                writer.close(true);
            }//
            ...
            if (log)
            {
                ...
                forAll(binDownCount, bini)
                {
                    Info<< "    " << token::TAB << bini
                        << token::TAB << delta
                        << token::TAB << binDownCount[bini]
                        << token::TAB << sumSortedVols[bini] << nl;
                    delta += deltaX;
                }
                Info<< endl;
