...
// * * * * * * * * * * * * * * * * * * * * * * * * * * interFoamPlus additive * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
        ...
        nu_lam = mixture.nu(); 						// Or const volScalarField& nu_lam = (turbulence->nu());
              
        UPrime = (U - UMean);                    			// Resolved velocity fluctuation vector
        pPrime = (p_rgh - p_rghMean);            			// Resolved pressure fluctuation
        
        SGSstress = -2.0*rho*nut*symm(fvc::grad(U));			// In fact it is Tau*, which is the trace free of Tau
        stressTensor = 2.0*rho*nu_lam*symm(fvc::grad(U));		// It is essential for vorticity equation calculations
        ...
        viscDiff = 2.0*rho*nu_lam*(strainTensor & UPrime);   		// Viscous diffusion +++ divergence operator is applied at the end of simulation through "interFoamPost"
        viscDiss = -2.0*rho*nu_lam*(strainTensor && strainTensor);      // Viscous dissipation
        SGSDiff = -rho*(UPrime & R);                          		// SGS diffusion +++ divergence operator is applied at the end of simulation through "interFoamPost"
        SGSDif2 = -UPrime & SGSstress;                     	        // An alternative for the above term. "SGSstress" is actually the trace free of the "R" tensor
        SGSDiss = SGSstress && strainTensor;                         	// SGS dissipation
